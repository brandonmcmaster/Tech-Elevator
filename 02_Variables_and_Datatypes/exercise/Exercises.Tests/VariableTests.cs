        ...
        private void AssertLastVarCorrectType(Exercise exercise, DataType acceptableType)
        {
            ...
            switch (acceptableType)
            {
                case DataType.WholeNumber:
                    ...
                    {
                        Assert.Fail("Your last variable is not the correct type for this problem. We need to store a whole number.");
                    }
                    break;
                case DataType.Floating:
                    if (!FloatingTypes.Any(type => lastVarDec.DataType == type))
                    {
                        Assert.Fail("Your last variable is not the correct type for this problem. We need to store a number with a decimal point.");
                    }
                    break;
                case DataType.String:
                    if (!StringTypes.Any(type => lastVarDec.DataType == type))
                    {
                        Assert.Fail("Your last variable is not the correct type for this problem. We need to store some text.");
                    }
                    break;
            }
        }
        ...
        private void AssertCorrectDecimalResult(Exercise exercise, object expectedValue, object delta)
        {
            ...
            else if (GetLastVarType(exercise) == "decimal")
            {
                ...
                if (!(Math.Abs(decActualValue - decExpectedValue) <= Math.Abs(decDelta))) {
                    Assert.Fail($"Excpected a difference no greater than <{decDelta}> between expected value <{decExpectedValue}> and actual <{decActualValue}>.");
                }
            }
            else
            {
