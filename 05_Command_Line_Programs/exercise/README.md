...
3. To verify that your application works as expected, select the project name from the drop-down in the menu bar that's displayed next to the execute button, and then click the button to run the program. Alternatively, you can right-click the project name, then select `Debug -> Start Debugging`, which displays the console for your application.
