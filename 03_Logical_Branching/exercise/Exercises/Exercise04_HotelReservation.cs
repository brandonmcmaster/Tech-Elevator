...
namespace TechElevator.Exercises.LogicalBranching
{
    ...
    public class HotelReservation
    {
        ...
        {
            if (numberOfNights >= 3 && includesParking)
            {
                return (DiscountRate + ParkingRate) * numberOfNights;
            }
            else if (numberOfNights >= 3)
                {
                return DiscountRate * numberOfNights;
            }
            else if (numberOfNights <= 2 && includesParking)
            {
                return (DailyRate + ParkingRate) * numberOfNights;
            }
            else if (numberOfNights <=2)
            {
                return DailyRate * numberOfNights;
        ...
        {
            double stayTotal = CalculateStayTotal(numberOfNights, includesParking);
            if (includesLateCheckout)
            {
                return stayTotal + LateCheckoutFee;
            }
            else
            {
                return stayTotal;
